...
const MyPage = () => {
    ...
    // 의사 난수 생성 함수 (간단한 해시 함수)
    const pseudoRandom = (seed) => {
        const values = [74, 113, 152, 191, 230];
        const hash = Math.abs(Math.sin(seed) * 10000);
        return values[Math.floor(hash % values.length)];
    };

    // RGB 색상 생성 함수
    const getRandomColor = (index) => {
        const r = pseudoRandom(index);
        const g = pseudoRandom(index + 1);
        const b = pseudoRandom(index + 2);
        return `rgb(${r}, ${g}, ${b})`;
    };
            ...
            <>
                ...
                <div className="bg-white top-0 pb-[33px] ml-[276px] mt-[104px] flex flex-col flex-grow gap-[120px] items-start justify-start shrink-0 h-auto relative">
                    <div className="flex flex-col gap-[39px] items-center justify-start flex-grow pt-8 w-full h-auto relative ">
                        ...
                        <div className="bg-white rounded-[10px] border-solid border-[#d9d9d9] border py-[40px] flex flex-row justify-evenly items-center shrink-0 w-[80%] h-auto relative">
